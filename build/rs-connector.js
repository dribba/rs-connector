...
/***/ function(module, exports, __webpack_require__) {
	...
	var _Connector = __webpack_require__(32);
	...
	BuilderState.prototype.merge = function merge(thatState) {
	    ...
	    return allConnections.reduce(function (_ref, _ref2) {
	        var storesAcc = _ref.stores,
	            connectionsAcc = _ref.connections;
	        var store = _ref2.store,
	            keys = _ref2.keys;

	        if (storesAcc.indexOf(store) === -1) {
	            var _connect = new _models.ConnectStore(store, keys);

	            return new BuilderState(storesAcc.concat(store), connectionsAcc.concat(_connect));
	        } else {
	            // find the existing connection
	            var existingConnectionIdx = findIndex(connectionsAcc, function (conn) {
	                return conn.store === store;
	            });
	            var existingConnection = connectionsAcc[existingConnectionIdx];
	            // create new connection
	            var _connect2 = new _models.ConnectStore(store, existingConnection.keys.concat(keys));
	            // replace old connection with the new one
	            connectionsAcc[existingConnectionIdx] = _connect2;
	...
	ConnectBuilder.prototype.wrap = ConnectBuilder.prototype.toComponent = ConnectBuilder.prototype.connectComponent = function toComponent(Component) {
	    return (0, _Connector2.default)(Component, this.state.connections);
	};
	...
	function resolveKeyString(key) {
	    ...
	    return [new _models.ConnectKey(keyName, key)];
	}

	function resolveKeysArray(keys) {
	    var resolveArray = function resolveArray(_ref3) {
	        var _ref4 = _slicedToArray(_ref3, 2),
	            key = _ref4[0],
	            name = _ref4[1];

	        return new _models.ConnectKey(key, name);
	    };
	...
	function resolveKeyObject(keys) {
	    return Object.keys(keys).map(function (key) {
	        return new _models.ConnectKey(key, keys[key]);
	    });
	}
	...
	function connect(rsKeys, store) {
	    var allKeys = resolveKeys(rsKeys);
	    var connection = new _models.ConnectStore(store, allKeys);
...
/***/ function(module, exports, __webpack_require__) {
	...
	function createConnections(connections) {
	    ...
	    var toUniqueStores = function toUniqueStores(stores, conn) {
	        if (stores.indexOf(conn.store) === -1) return stores.concat(conn.store);else return stores;
	    };
	    var toConnectedStore = function toConnectedStore(store) {};
	    var stores = connections.reduce(toUniqueStores, []);
	    var keys = connections.stores.map(toConnectedStore);
	    return flatMap(connections, function (_ref) {
	        ...
	        return storeKeys.map(function (_ref3) {
	            var _ref4 = _slicedToArray(_ref3, 2),
	                stateKey = _ref4[0],
	                storeKey = _ref4[1];
	...
	function connectStores(connectedStores, init, update) {
	    var stopAllFns = [];
	    var initState = connectedStores.reduce(function (initialState, connection) {
	        var firstRun = true;
	        var stop = connection.store.autorun(function () {
	            var newState = connection.keys.reduce(function (accState, keyConnect) {
	    ...
	    return function () {
	        stopAllFns.forEach(function (fn) {
	            return fn();
	        });
	    };
	}

	function Connector(Component, connections) {
	    return function (_React$Component) {
	        ...
	        function _class(props) {
	            ...
	            _this.isCompositeComponent = typeof Component === 'function' && Component instanceof React.Component;

	            _this.stop = connectStores(connections || [], function (initial) {
	                _this.state = initial;
	            }, function (updated) {
